...
def get_args():
	...
	parser = PromptParser()
	parser.add_argument("-D", "--Data", default="./Data/", type=str, help="Data Directory", prompt=True)
	parser.add_argument("-M", "--Models", default="./Models/", type=str, help="Models Directory", prompt=True)
	parser.add_argument("-T", "--Train", default=True, type=str, help="Train New Model", prompt=True)
	parser.add_argument("-P", "--Predict", default=True, type=str, help="Predict using Trained Model", prompt=True)
	parser.add_argument("-L", "--loadedModel", default="CNN_BLSTM.h5", help="Pretrained Model Path", prompt=True)
	parser.add_argument("-G", "--Debug", default=False, help="Debug Mode", prompt=False)
...
def preprocess(X, to_tokenize):

	to_exclude = '!"#%&()*+,/:;<=>?@[\\]^_`|{}~\t\n'
	
	tokenizer = Tokenizer(filters=to_exclude, char_level=True, lower=False, oov_token='')
	X_clean = [re.sub(r'(['+to_tokenize+'])', r' ', x) for x in X]
	...
	payload_len = max([len(x) for x in X_seq])
	
	X_processed = pad_sequences(X_seq, maxlen=payload_len, padding='post', truncating='post')
	
	return X_processed, n_unique, payload_len, tokenizer
	
def build_model(n_unique, payload_len):
	
	#Padded Input
	char_in = Input(shape=(payload_len,))
	
	#Embed Characters
	emb_char = Embedding(input_dim=n_unique, output_dim=16, 
							   input_length=payload_len)(char_in)

	#1D Convolution + Pooling
	conv1 = Conv1D(kernel_size=4, filters=16, padding='same', activation='relu', strides=1)(emb_char)
	pool = MaxPooling1D(strides=2)(conv1)
	
	mask = Masking(mask_value=0)(pool)

	#LSTM Hidden Layer
	char_enc = LSTM(units=32, return_sequences=True, dropout=0.5,
									recurrent_dropout=0.5)(mask)

	#1D Spatial Dropout + BLSTM
	x = SpatialDropout1D(0.5)(char_enc)
	main_lstm = Bidirectional(LSTM(units=32, return_sequences=True, dropout=0.5,
								   recurrent_dropout=0.5))(x)
	#LSTM Stacked
	out_lstm = LSTM(units=16, return_sequences=False)(main_lstm)
...
def prediction(loaded_model, val_input, to_tokenize, tokenizer, payload_len):
	
	val_input_clean = [re.sub(r'(['+to_tokenize+'])', r' ', x) for x in [val_input]]
	
	X_val_seq = tokenizer.texts_to_sequences(val_input_clean)

	X_val_processed = pad_sequences(X_val_seq, maxlen=payload_len, padding='post', truncating='post')
...
def main():
	...
	X_train, X_test, y_train, y_test = train_test_split(X_processed, y, test_size=0.2, random_state=222)
	...
	if args.Train not in ['False','false','f','F']:
		...
		model = build_model(n_unique, payload_len)
		
		###################################################
		#TRAINING##########################################
		
		early_stop = EarlyStopping(monitor='val_loss', patience=2)
		checkpoint = ModelCheckpoint(filepath='./CNN_BLSTM_Checkpoint.hdf5', monitor='val_loss', verbose=1, save_best_only=True)
		callbacks_list = [checkpoint, early_stop]
		
		history = model.fit([X_train], y_train, batch_size=32, epochs=10, validation_split=0.2, callbacks=callbacks_list, verbose=1)
		...
		score, acc = model.evaluate([np.array(X_test)], np.array(y_test).reshape(len(y_test), 1), batch_size=32)
	...
	if args.Predict not in ['False','false','f','F']:
		...
		score, acc = loaded_model.evaluate([np.array(X_test)], np.array(y_test).reshape(len(y_test), 1), batch_size=32)
		...
		res =  prediction(loaded_model, val_input, to_tokenize, tokenizer, payload_len)		
